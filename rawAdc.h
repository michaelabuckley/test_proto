...
int16_t analogDifferential(uint8_t pin_pos,uint8_t pin_neg) {
  ...
  //Serial.printf("sample pins  %d %d\n", pin_pos, pin_neg);
  if((g_APinDescription[pin_neg].ulADCChannelNumber>0x10) && (pin_neg<ADC_PIN_GND)) { // if the negative pin is out of bounds
    Serial.printf("wrong pin %d %d %d\n", pin_neg, g_APinDescription[pin_neg].ulADCChannelNumber, ADC_PIN_GND);
    return 0;
  }
